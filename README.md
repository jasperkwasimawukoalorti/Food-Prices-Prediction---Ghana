...
2. Download `wfp_food_prices_gha.csv` from [data.humdata.org](https://data.humdata.org/dataset/wfp-food-prices-for-ghana) and place it in `data/`.
   - **Check for an HXL tag row first** (is row 2 real data, or does it look like `#date,#adm1+name,...`?). If there's no HXL row, do **not** pass `skip=1` to `read_csv_auto` — it will corrupt the header. `sql/01_staging.sql` already reflects the no-skip version for this dataset.
...
├── .gitignore
├── data/
│   ├── wfp_food_prices_gha.csv       # full raw file — git-ignored, download it yourself
